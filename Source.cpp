...
float findAngle(float x1, float y1, float x2, float y2) {
	...
	float angle = atan2((y2 - y1), (x2 - x1)) * 180 / PHI;

	if (angle >= -90 && angle < 0) {
		angle = (sqrt(pow(angle, 2))) + 90;
	}
	else if (angle >= 0 && angle < 90) {
		angle = (angle - 180) + 90;
		angle = (sqrt(pow(angle, 2))) + 360;
	}
...
int main()
{
	...
	for (int i = 1; i <= 10;i++) {
		...
		imwrite("VISUAL2AC\\RegionFilter.jpg", res);

		res = coLinear(res);
		//namedWindow("coLinear", WINDOW_AUTOSIZE);
		//imshow("coLinear", res);
		imwrite("VISUAL2AC\\Collinear.jpg", res);
	...
	//Metode ekstraksi yang membentuk kontour dan menghitung jarak antar dinding sistole menuju diastole (Metode Simpson)
	//ExtractionMethodIV();
...
void GetGoodFeature() {
	...
	if (kondisi1 < kondisi2) {
		...
		for (int i = 1; i <= jumlah; i++) {
			...
			hasil = garis & res;
			temp3 = 0;
			//MENDAPATKAN COORDINATE DARI DINDING JANTUNG KANAN KE KIRI (GOODFEATURE KANAN)
			for (int x = hasil.cols - 1; x > 0; x--) {
				for (int y = hasil.rows - 1; y > 0; y--) {
					if (hasil.at<uchar>(y, x) > 0) {
						temp1++;
			...
			//MENDAPATKAN COORDINATE DARI DINDING JANTUNG KIRI KE KANAN (GOODFEATUERE KIRI)
			for (int x = 0; x < hasil.cols; x++) {
				for (int y = 0; y < hasil.rows; y++) {
					if (hasil.at<uchar>(y, x) > 0) {
						temp2++;
...
void opticalFlowCalc() {
	float thresh_diff = 20.0;
	float lengthNorm, avgNeigh, angleNorm;
	float length[4];
	Point P1;
	vector<uchar> status;
	...
	for (int i = 0; i < 9; i++) {
		...
		//UJI COBA MEMBERIKAN NOISE PADA KOORDINATE X,Y
		/*
		for (int j = 0; j < goodFeatures[i].size();j++) {

			if (j == 0 && i == 3) {
				goodFeatures[i + 1][j].x = goodFeatures[i + 1][j].x + 50;
			}
			else if (j == 1 && i == 3) {
				goodFeatures[i + 1][j].x = goodFeatures[i + 1][j].x + 50;
			}
		...
		*/
		...
		//##################### NORMALISASI POINT (X,Y) DILUAR NILAI THRESHOLD #####################
		//####### NORMALISASI POINT (X,Y) 1-10 & 13-22 ##########
		//Menggunakan 4X Perulangan Normalisasi Point
		for (int k = 0; k < 4;k++) {
			for (int j = 0;j < goodFeatures[i].size();j++) {
				length[0] = (sqrt(pow(goodFeatures[i][j].x - goodFeatures[i + 1][j].x, 2) + pow(goodFeatures[i][j].y - goodFeatures[i + 1][j].y, 2))) / (valnorm) * 100;
				if (length[0] > thresh_diff) {
					if (j > 0 && j < 11 || j > 12 && j < 23) {
						length[1] = (sqrt(pow(goodFeatures[i][j - 1].x - goodFeatures[i + 1][j - 1].x, 2) + pow(goodFeatures[i][j - 1].y - goodFeatures[i + 1][j - 1].y, 2))) / (valnorm) * 100;
						length[2] = (sqrt(pow(goodFeatures[i][j + 1].x - goodFeatures[i + 1][j + 1].x, 2) + pow(goodFeatures[i][j + 1].y - goodFeatures[i + 1][j + 1].y, 2))) / (valnorm) * 100;

						if (length[1] < thresh_diff) {
							length[3] = (sqrt(pow(goodFeatures[i][j - 1].x - goodFeatures[i + 1][j - 1].x, 2) + pow(goodFeatures[i][j - 1].y - goodFeatures[i + 1][j - 1].y, 2)));
							angleNorm = findAngle(goodFeatures[i][j - 1].x, goodFeatures[i][j - 1].y, goodFeatures[i + 1][j - 1].x, goodFeatures[i + 1][j - 1].y);
							double s = sin(angleNorm*CV_PI / 180); double c = cos(angleNorm*CV_PI / 180);
							Point P(goodFeatures[i][j].x + s * length[3], goodFeatures[i][j].y + c * length[3]);
							goodFeatures[i + 1][j] = P;

						}
						else if (length[2] < thresh_diff) {
							length[3] = (sqrt(pow(goodFeatures[i][j + 1].x - goodFeatures[i + 1][j + 1].x, 2) + pow(goodFeatures[i][j + 1].y - goodFeatures[i + 1][j + 1].y, 2)));
							angleNorm = findAngle(goodFeatures[i][j + 1].x, goodFeatures[i][j + 1].y, goodFeatures[i + 1][j + 1].x, goodFeatures[i + 1][j + 1].y);
							double s = sin(angleNorm*CV_PI / 180); double c = cos(angleNorm*CV_PI / 180);
							Point P(goodFeatures[i][j].x + s * length[3], goodFeatures[i][j].y + c * length[3]);
		...
		//####### NORMALISASI POINT (X,Y) 0 dan 11 & 12 dan 23 ##########
		for (int j = 0;j < goodFeatures[i].size();j++) {
			length[0] = (sqrt(pow(goodFeatures[i][j].x - goodFeatures[i + 1][j].x, 2) + pow(goodFeatures[i][j].y - goodFeatures[i + 1][j].y, 2))) / (valnorm) * 100;
			if (length[0] > thresh_diff) {
				if (j == 0 || j == 12) {
	...
	//##################### VISUALISASI GOODFEATURE #####################
	for (int i = 0; i < 24; i++) {
		circle(images[0], goodFeatures[0][i], 1, Scalar(255, 255, 255), 2, 8, 0);
	}
	imwrite("VISUAL2AC\\Goodfeature.jpg", images[0]);

	//##################### VISUALISASI TRACKING #####################
	/*
	for (int i = 0; i < 24; i++) {
		line(images[0], goodFeatures[0][i], goodFeatures[9][i], Scalar(0, 255, 255), 2, 8);
	}
	//imshow("HASIL TRACKING", images[0]);
	imwrite("Tracking2ac.jpg", images[0]);
	*/

	/*
	for (int i = 0; i < 9; i++) {
		for (int j = 0;j < goodFeatures[i].size();j++) {
			cout << lengthDiffirence[i][j]<<"----------------"<<goodFeatures[i+1][j] << endl;
		}
		cout << "###############" << endl;
	}
...
/* JIKA HASIL PERGERAKAN ARAH BURUK, GUNAKAN METODE YANG LAIN (ILUSTRASI AMBIL TITIK PERGERAKAN
SISI KANAN, SELANJUTNYA AMBIL TITIK AWAL SEBELAH KIRI (DAN SEBALIKNYA) */
...
void ExtractionMethodI() {
	...
	std::ofstream myfile;
	myfile.open("M1F1_2AC.csv", std::ios_base::app);
	for (int j = 0; j < (jumlah * 2) - 1;j++) {
		myfile << to_string(pf[j]) + "," + to_string(nf[j]) + "," + to_string(pm[j]) + "," + to_string(nm[j]) + ",";
		if (j == (jumlah * 2) - 2) {
			myfile << to_string(pf[j + 1]) + "," + to_string(nf[j + 1]) + "," + to_string(pm[j + 1]) + "," + to_string(nm[j + 1]) << endl;
		}
	}
	myfile.close();
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD II
	myfile.open("M2F1.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah / 2) - 1;j++) {
		myfile << to_string(pf1[j]) + "," + to_string(nf1[j]) + "," + to_string(pm1[j]) + "," + to_string(nm1[j]) + ",";
		if (j == ((jumlah) / 2) - 2) {
			myfile << to_string(pf1[j + 1]) + "," + to_string(nf1[j + 1]) + "," + to_string(pm1[j + 1]) + "," + to_string(nm1[j + 1]) << endl;
		}
	}
	myfile.close();
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD III
	myfile.open("M3F1.txt", std::ios_base::app);
	for (int j = 0; j < (9 - 1);j++) {
		myfile << to_string(pf2[j]) + "," + to_string(nf2[j]) + "," + to_string(pm2[j]) + "," + to_string(nm2[j]) + ",";
		if (j == (9 - 2)) {
			myfile << to_string(pf2[j + 1]) + "," + to_string(nf2[j + 1]) + "," + to_string(pm2[j + 1]) + "," + to_string(nm2[j + 1]) << endl;
		}
	}
	myfile.close();
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD IV
	myfile.open("M4F1.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah * 2) - 1;j++) {
		myfile << to_string(flow[j]) + "," + to_string(move[j]) + ",";
		if (j == (jumlah * 2) - 2) {
			myfile << to_string(flow[j + 1]) + "," + to_string(move[j + 1]) << endl;
		}
	}
	myfile.close();
	*/
...
void trackvizualization() {
	Mat source = Mat::zeros(res.size(), res.type());
	Mat goodfeatureimg = Mat::zeros(images[0].size(), images[0].type());

	/*
	//Visualisasi GoodFeature
	images[0].copyTo(goodfeatureimg);
	for (int i = 0; i < jumlah*2; i++) {
		cv::String filename = format("Goodfeature2ac.jpg");
		circle(goodfeatureimg, goodFeatures[0][i], 1, Scalar(255, 255, 255), 2, 8, 0);
		imwrite(filename, goodfeatureimg);
	}


	for (int i = 0; i < jumlah*2; i++) {
		line(goodfeatureimg, goodFeatures[0][i], goodFeatures[9][i], Scalar(0, 255, 255), 2, 8);
	}
	//imshow("HASIL TRACKING", images[0]);
	imwrite("Tracking2ac.jpg", goodfeatureimg);
	*/

	//Visualisasi Tracking
	vector<pair<float, float>> vect1[10];vector<pair<float, float>> vect2[10];
	Point2f coordinate[50][50];
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < (jumlah * 2) - 1; j++) {
			line(images[i], coordinate[i][j], coordinate[i][j + 1], Scalar(0, 255, 0), 2, 8);
		}
	}
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < jumlah * 2; j++) {
			circle(images[i], goodFeatures[i][j], 1, Scalar(255, 255, 255), 2, 8, 0);
		}
		cv::String filename = format("TRACKING2AC\\PIC %d.jpg", i);
...
void ExtractionMethodII() {
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD I
	std::ofstream myfile;
	myfile.open("M1F2.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah * 2) - 1;j++) {
		myfile << to_string(pfUP[j]) + "," + to_string(pfDOWN[j]) + "," + to_string(nfUP[j]) + "," + to_string(nfDOWN[j]) + "," + to_string(pmUP[j]) + "," + to_string(pmDOWN[j]) + "," + to_string(nmUP[j]) + "," + to_string(nmDOWN[j]) + ",";
		if (j == (jumlah * 2) - 2) {
			myfile << to_string(pfUP[j + 1]) + "," + to_string(pfDOWN[j + 1]) + "," + to_string(nfUP[j + 1]) + "," + to_string(nfDOWN[j + 1]) + "," + to_string(pmUP[j + 1]) + "," + to_string(pmDOWN[j + 1]) + "," + to_string(nmUP[j + 1]) + "," + to_string(nmDOWN[j + 1]) << endl;
		}
	}
	myfile.close();
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD II
	myfile.open("M2F2.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah / 2) - 1;j++) {
		myfile << to_string(pfUP[j]) + "," + to_string(pfDOWN[j]) + "," + to_string(nfUP[j]) + "," + to_string(nfDOWN[j]) + "," + to_string(pmUP[j]) + "," + to_string(pmDOWN[j]) + "," + to_string(nmUP[j]) + "," + to_string(nmDOWN[j]) + ",";
		if (j == ((jumlah) / 2) - 2) {
			myfile << to_string(pfUP[j + 1]) + "," + to_string(pfDOWN[j + 1]) + "," + to_string(nfUP[j + 1]) + "," + to_string(nfDOWN[j + 1]) + "," + to_string(pmUP[j + 1]) + "," + to_string(pmDOWN[j + 1]) + "," + to_string(nmUP[j + 1]) + "," + to_string(nmDOWN[j + 1]) << endl;
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD III
	myfile.open("M3F2.txt", std::ios_base::app);
	for (int j = 0; j < (9 - 1);j++) {
		myfile << to_string(pfUP[j]) + "," + to_string(pfDOWN[j]) + "," + to_string(nfUP[j]) + "," + to_string(nfDOWN[j]) + "," + to_string(pmUP[j]) + "," + to_string(pmDOWN[j]) + "," + to_string(nmUP[j]) + "," + to_string(nmDOWN[j]) + ",";
		if (j == (9 - 2)) {
			myfile << to_string(pfUP[j + 1]) + "," + to_string(pfDOWN[j + 1]) + "," + to_string(nfUP[j + 1]) + "," + to_string(nfDOWN[j + 1]) + "," + to_string(pmUP[j + 1]) + "," + to_string(pmDOWN[j + 1]) + "," + to_string(nmUP[j + 1]) + "," + to_string(nmDOWN[j + 1]) << endl;
...
void ExtractionMethodIII() {
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD I
	std::ofstream myfile;
	myfile.open("M1F3.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah * 2) - 1;j++) {
		myfile << to_string(flow[j]) + "," + to_string(move[j]) + ",";
		if (j == (jumlah * 2) - 2) {
			myfile << to_string(flow[j + 1]) + "," + to_string(move[j + 1]) << endl;
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD II
	myfile.open("M2F3.txt", std::ios_base::app);
	for (int j = 0; j < (jumlah / 2) - 1;j++) {
		myfile << to_string(flow1[j]) + "," + to_string(move1[j]) + ",";
		if (j == (jumlah / 2) - 2) {
			myfile << to_string(flow1[j + 1]) + "," + to_string(move1[j + 1]) << endl;
		}
	}
	myfile.close();
	...
	//MENYIMPAN FEATURE EXTRACTION METHOD III
	myfile.open("M3F3.txt", std::ios_base::app);
	for (int j = 0; j < (9 - 1);j++) {
		myfile << to_string(flow2[j]) + "," + to_string(move2[j]) + ",";
		if (j == (9 - 2)) {
			myfile << to_string(flow2[j + 1]) + "," + to_string(move2[j + 1]) << endl;
		}
	}
	myfile.close();
}
...
void ExtractionMethodIV() {
	...
	for (int i = 0; i < 10; i++) {
		...
		if (kondisi1 < kondisi2) {
			...
			valnormalisasi = sqrt(pow(rect_points[2].x - rect_points[3].x, 2) + pow(rect_points[2].y - rect_points[3].y, 2));
			...
			degreeA = -(degree);
			//Mendapatkan nilai untuk normalisasi
			valnormalisasi = sqrt(pow(rect_points[1].x - rect_points[2].x, 2) + pow(rect_points[1].y - rect_points[2].y, 2));
			//Mencari titik tengah dari kontur
			Moments m = moments(kontur, true);
			Point cen(m.m10 / m.m00, m.m01 / m.m00);
			double s = sin(degreeA*CV_PI / 180); double c = cos(degreeA*CV_PI / 180);
		...
		for (int j = 1; j <= garisan; j++) {
			...
			float length = (sqrt(pow(koordinat2[j][i].x - koordinat2[j + garisan][i].x, 2) + pow(koordinat2[j][i].y - koordinat2[j + garisan][i].y, 2))) / (valnorm) * 100;
			lengthdif[i].push_back(length);
		}
	}

	std::ofstream myfile;
	myfile.open("M1F4.txt", std::ios_base::app);
	for (int j = 0; j < (garisan - 1); j++) {
		float length = lengthdif[0][j] - lengthdif[9][j];
		myfile << to_string(length) + ",";
		if (j == (garisan - 2)) {
			float length = lengthdif[0][j + 1] - lengthdif[9][j + 1];
			myfile << to_string(length) << endl;
		}
	}
	myfile.close();
}
